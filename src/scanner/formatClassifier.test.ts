...
describe('matchFormat', () => {
    it('matches .cursor/rules/*.mdc as cursor-rules', () => {
        expect(matchFormat('.cursor/rules/my-rule.mdc', 'my-rule.mdc', '.mdc')?.id).toBe('cursor-rules');
    });
    ...
    it('matches .augment-guidelines as augment-guidelines', () => {
        expect(matchFormat('.augment-guidelines', '.augment-guidelines', '')?.id).toBe('augment-guidelines');
    });
...
describe('classify — trigger mapping', () => {
    it('cursor-rules: alwaysApply true → always', () => {
        const r = classify(`${WS}/.cursor/rules/r.mdc`, '---\nalwaysApply: true\n---\nUse TypeScript strict mode for all project files', 100, NOW, WS);
    ...
    it('cursor-rules: no frontmatter → manual', () => {
        const r = classify(`${WS}/.cursor/rules/r.mdc`, 'Use TypeScript strict mode for all project files in the repo', 100, NOW, WS);
        expect(r?.trigger).toBe('manual');
    });

    it('cursorrules: standalone → always', () => {
        const r = classify(`${WS}/.cursorrules`, 'Use TypeScript strict mode for all project files in the repo', 100, NOW, WS);
        expect(r?.format).toBe('cursorrules');
        expect(r?.trigger).toBe('always');
    });

    it('windsurf-rules: trigger always_on → always', () => {
        const r = classify(`${WS}/.windsurf/rules/r.md`, '---\ntrigger: always_on\n---\nUse TypeScript strict mode for all project files', 100, NOW, WS);
        expect(r?.trigger).toBe('always');
    });

    it('windsurf-rules: trigger glob + globs → glob', () => {
        const r = classify(`${WS}/.windsurf/rules/r.md`, '---\ntrigger: glob\nglobs:\n  - "*.ts"\n---\nUse TypeScript strict mode for all files', 100, NOW, WS);
        expect(r?.trigger).toBe('glob');
        expect(r?.globs).toEqual(['*.ts']);
    });

    it('windsurf-rules: no frontmatter → always', () => {
        const r = classify(`${WS}/.windsurf/rules/r.md`, 'Use TypeScript strict mode for all project files in the repo', 100, NOW, WS);
        expect(r?.trigger).toBe('always');
    });

    it('augment-rules: type always_apply → always', () => {
        const r = classify(`${WS}/.augment/rules/r.md`, '---\ntype: always_apply\n---\nUse TypeScript strict mode for all project files', 100, NOW, WS);
        expect(r?.trigger).toBe('always');
    });
    ...
    it('claude-rules: paths → glob', () => {
        const r = classify(`${WS}/.claude/rules/r.md`, '---\npaths:\n  - "src/**"\n---\nUse TypeScript strict mode for all project files', 100, NOW, WS);
        expect(r?.trigger).toBe('glob');
        expect(r?.globs).toEqual(['src/**']);
    });

    it('claude-rules: no frontmatter → always', () => {
        const r = classify(`${WS}/.claude/rules/r.md`, 'Use TypeScript strict mode for all project files in the repo', 100, NOW, WS);
        expect(r?.trigger).toBe('always');
    });

    it('kiro: inclusion always → always', () => {
        const r = classify(`${WS}/.kiro/steering/r.md`, '---\ninclusion: always\n---\nUse TypeScript strict mode for all project files', 100, NOW, WS);
        expect(r?.trigger).toBe('always');
    });

    it('kiro: inclusion fileMatch → glob', () => {
        const r = classify(`${WS}/.kiro/steering/r.md`, '---\ninclusion: fileMatch\nfileMatchPattern:\n  - "*.ts"\n---\nUse TypeScript strict mode', 100, NOW, WS);
        expect(r?.trigger).toBe('glob');
        expect(r?.globs).toEqual(['*.ts']);
    });

    it('claude-md at root → always', () => {
        const r = classify(`${WS}/CLAUDE.md`, '# Rules\n\nUse TypeScript strict mode for all project files in the repo', 100, NOW, WS);
    ...
    it('claude-md in subdirectory → glob', () => {
        const r = classify(`${WS}/src/components/CLAUDE.md`, 'Component rules for TypeScript strict mode for all files in project', 100, NOW, WS);
        expect(r?.format).toBe('claude-md');
        expect(r?.trigger).toBe('glob');
...
describe('classify — frontmatter', () => {
    ...
    it('handles no frontmatter', () => {
        const r = classify(`${WS}/.cursor/rules/r.mdc`, 'Just body content no frontmatter use TypeScript strict mode for all files', 100, NOW, WS);
...
describe('classify — description', () => {
    ...
    it('falls back to first heading when no frontmatter description', () => {
        const r = classify(`${WS}/CLAUDE.md`, '# Project Rules\n\nUse TypeScript strict mode for all project files in repo', 100, NOW, WS);
        expect(r?.description).toBe('Project Rules');
    });

    it('returns undefined when no description or heading', () => {
        const r = classify(`${WS}/CLAUDE.md`, 'Just plain text without heading use TypeScript strict mode for all files', 100, NOW, WS);
...
describe('classify — links', () => {
    it('extracts markdown links', () => {
        const r = classify(`${WS}/CLAUDE.md`, 'See [reference](./docs/rules.md) for more details about the project rules', 100, NOW, WS);
        expect(r?.links).toContainEqual({ patternId: 'markdown-link', target: './docs/rules.md' });
    });

    it('extracts backtick paths', () => {
        const r = classify(`${WS}/CLAUDE.md`, 'Check the config in `config/settings.json` for project settings and rules', 100, NOW, WS);
    ...
    it('skips URLs', () => {
        const r = classify(`${WS}/CLAUDE.md`, 'Visit [docs](https://example.com/docs) for more project info and rules', 100, NOW, WS);
        expect(r?.links).toHaveLength(0);
    });

    it('skips anchors', () => {
        const r = classify(`${WS}/CLAUDE.md`, 'See [section](#overview) for more info about the project rules overview', 100, NOW, WS);
        expect(r?.links).toHaveLength(0);
    });
});
...
describe('classify — hashing', () => {
    it('computes bodyHash and contentHash', () => {
        const r = classify(`${WS}/CLAUDE.md`, 'Use TypeScript strict mode for all project files in the repo', 100, NOW, WS);
